...
public class FakeBotManager {
    ...
    public void addBot(String name, FabricClientCommandSource source) {
        ...
        if (world == null) {
            source.sendError(Component.literal("You must be in a world to spawn a bot!"));
            return;
        }

        if (activeBots.containsKey(name.toLowerCase())) {
            source.sendError(Component.literal("A bot named '" + name + "' already exists!"));
            return;
        }

        Vec3 pos = source.getClient().player.position();
        FakeBot bot = new FakeBot(name, world, pos);
        activeBots.put(name.toLowerCase(), bot);
        source.sendFeedback(Component.literal("§aSpawned client-side bot '" + name + "' at your location."));
    }
    ...
    public void removeBot(String name, FabricClientCommandSource source) {
        ...
        if (bot != null) {
            ...
            source.sendFeedback(Component.literal("§aRemoved client-side bot '" + name + "'."));
        } else {
    ...
    public void listBots(FabricClientCommandSource source) {
        if (activeBots.isEmpty()) {
            source.sendFeedback(Component.literal("§eNo active client-side bots."));
            return;
        }
        source.sendFeedback(Component.literal("§eActive client-side bots: §a" + String.join(", ", activeBots.keySet())));
    }
    ...
    public void setStandStill(String name, boolean val, FabricClientCommandSource source) {
        ...
        if (bot != null) {
            ...
            source.sendFeedback(Component.literal("§aBot '" + name + "' standstill: " + (val ? "§2ON" : "§cOFF")));
        } else {
            source.sendError(Component.literal("Bot '" + name + "' not found."));
        }
    }
    ...
    public void setWander(String name, boolean val, FabricClientCommandSource source) {
        ...
        if (bot != null) {
            ...
            source.sendFeedback(Component.literal("§aBot '" + name + "' wander: " + (val ? "§2ON" : "§cOFF")));
        } else {
            source.sendError(Component.literal("Bot '" + name + "' not found."));
        }
    }
